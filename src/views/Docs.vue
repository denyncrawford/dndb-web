<template>
  <page bg="bg-black">
    <div
      class="-mt-10 flex-col min-h-screen items-center justify-center flex text-center"
    >
      <h1 class="text-main-color text-3xl sm:text-5xl mb-10">
        DnDB Documentation
      </h1>
      <p class="flex mb-10">
        <a href="https://nest.land/package/dndb"
          ><img src="https://nest.land/badge.svg" alt="nest badge"
        /></a>
        <img
          ...
          nii7lr0wn=""
        />
        <a class="ml-5" href="https://deno.land/x/dndb"
          ><img
            src="https://img.shields.io/badge/deno-v1.1.3-green.svg?style=flat-square&amp;logo=deno"
            alt="(Deno)"
        /></a>
        <a
          class="ml-5"
          href="https://github.com/denyncrawford/crawford/blob/master/LICENSE"
          ><img
      ...
      <p class="max-w-prose">
        DnDB is a javascript persistent database written for deno and available
        for Typescript.
      </p>
      <p class="max-w-prose">
        Inspired by NeDB, DnDB is a robust but minimalist database engine
        written on JS that uses/emulates the Mongo query API to edit and find
    ...
    <div class="sm:mx-32 mb-32 flex flex-col sm:flex-row">
      <div class="sm:w-96 mr-10">
        <div class="py-10 sm:p-10 rounded-2xl sticky top-24">
          <h1 class="text-lg font-bold text-main-color mb-5">Contents</h1>
          <ul>
            <li class="mb-1"><a href="#usage">Usage</a></li>
            <li class="mb-1"><a href="#importing">Importing</a></li>
            <li class="mb-1">
              <a href="#instatiating-a-collection"
                >Instantiating a collection</a
              >
            </li>
            <li class="mb-1">
              <a href="#inserting-documents">API</a>
              <ul class="pl-4">
                <li class="mb-1">
      ...
      <div class="markdown" data-v-a5f3b890="">
        ...
        <p>
          DnDB works by instantiating collections stored in a specific file
          given by the user in the instance configuration. By default the
          created instance exposes the necessary methods to work the datastore.
        </p>
        <p>
          All the api methods are asynchronous by default, so they return
          promises, but it's also possible to pass a callback to them to work in
          a traditional Mongo way.
        </p>
        <blockquote>
          <p>
            <em>Notice</em>: This project is under development, so it is subject
        ...
        <pre
          v-highlightjs
        ><code class="javascript bg-black">import Datastore from 'https://deno.land/x/dndb@{{version}}/mod.ts'
</code></pre>
        <p><strong>From nest.land</strong></p>
        <pre
          v-highlightjs
        ><code class="javascript bg-black">import Datastore from 'https://x.nest.land/dndb@{{version}}/mod.ts'
</code></pre>
...
const db = new Datastore({ filename:"./database.db", autoload: true })
        ...
        <p>
          When you instantiate a collection you can pass it a config object with
          a couple of options:
        </p>
        <ul>
          <li>
            <p>
              <code>filename</code>: The filename is the absolute path to your
              traget file. If no filename is provided, DnDB will automatically
              create one in the current working directory, and if a full path is
              not specified, it will resolve the file name within the CWD.
            </p>
          </li>
          <li>
            <p>
              <code>autoload</code>: The autoload option runs the
              <code>loadDatabase</code> method, which creates the persistent file
        ...
        <blockquote>
          <p>
            <em>Notice</em>: The configuration content is currently in alpha,
            more options will be available soon.
          </p>
        </blockquote>
        <h2 id="inserting-documents">🖋️ Inserting documents</h2>
        <p>
          All data types are allowed, but field names starting with '$' are
          reserved for data querying.
        </p>
        <p>
          If the document does not contain an _id field, DnDB will automatically
          generated one for you (a RFC4122 UUID alphanumerical string). The _id
          of a document, once set, shouldn't be modified.
        </p>
...
db.insert(obj, (insertion) =&gt; {
  // ...foo(insetion)
        ...
        <ul>
          ...
          <li>
            <code>callback</code>(optional): The callback function to get the
            data inserted.
          </li>
        </ul>
        <p>
          You can also insert several documents at the same time by wrapping
          them in an array.
        </p>
...
db.insert([ { name: 'denyn' }, { name: foo } ], (insertion) =&gt; {
  // ...foo(insetion)
        ...
        <ul>
          <li>
            <p>
              <code>find</code>: Finds all the documents that match the query.
            </p>
            <ul>
          ...
          <li>
            <p>
              <code>findOne</code>: Finds the first document that matches the
              query.
            </p>
            <ul>
        ...
        <p>
          You can select documents based on field equality or use comparison
          operators (<code>$lt</code>, <code>$lte</code>, <code>$gt</code>,
          <code>$gte</code>, <code>$in</code>, <code>$nin</code>,
          <code>$neq</code>). You can also use logical operators
          <code>$or</code>, <code>$and</code>, <code>$not</code> and
          <code>$eq</code>. See below for the syntax.
        </p>
        <p>
          You can use regular expressions in two ways: in basic querying in
          place of a string, or with the $regex operator.
        </p>
        <p><em>Example of document querying:</em></p>
        <blockquote>
          <p>
            <em>Notice</em>: See all rules and operators list
            <a href="https://www.npmjs.com/package/mingo">here</a>
          </p>
        </blockquote>
        <pre v-highlightjs><code class="javascript bg-black">
...
let docs = await db.find( { "list.games.0": "Doom" })
        ...
        <blockquote>
          <p>
            ...
            <a href="https://github.com/louischatriot/nedb/#basic-querying"
              >read this</a
            >
          </p>
        </blockquote>
        ...
        <p>
          You can give <code>find</code> and <code>findOne</code> an optional
          second argument, projections. The syntax is the same as MongoDB:
          <code>{ a: 1, b: 1 }</code> to return only the a and b fields,
          <code>{ a: 0, b: 0 }</code> to omit these two fields. You cannot use
          both modes at the time, except for _id, which is by default always
          returned and which you can choose to omit. You can project on nested
          documents.
        </p>
        <blockquote>
          <p>
...
db.find({ planet: 'Mars' }, { planet: 1, system: 1 }, function (docs) {
  // docs is [{ planet: 'Mars', system: 'solar', _id: 'id1' }]
        ...
        <h2 id="updating-documents">🖌️ Updating documents</h2>
        ...
        <p>
          The update method follows the same query rules as in
          <code>find</code> and <code>findOne</code> at first argument to get
          the update target document and as a second agument it receives the
          aggregation operators that modifies the matching fileds values ​​by
        ...
        <pre
          v-highlightjs
        ><code class="javascript bg-black">db.update({ name: 'denyn' }, { $set: { pet: 'Boots' } }, (update) =&gt; {
  // ...foo(update)
});
...
let update = await db.update({ name: 'denyn' }, { $set: { pet: 'Boots' } })
        ...
        <p>
          The remove method follows the same query rules as in
          <code>find</code> and <code>findOne</code> at first argument, it will
          remove all the documents that matches the query.
        </p>
        <pre
          v-highlightjs
        ><code class="javascript bg-black">db.remove({ _id: 'id2' }, function (newdoc) {
  // ...foo(newDoc)
});
